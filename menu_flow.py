...
def menu_flow(
    ...
    while True:
        options = [
            ...
            f"lands must produce deck colours   ({"yes" if settings["lands_produce_colours"] else "no"})",
        ...
        if(menu_entry_index == 4):
            settings["lands_produce_colours"] = validate_input(f"Choose whether lands must produce colours in the deck's colour identity (currently {"yes" if settings["lands_produce_colours"] else "no"}) (y/N): ", validate_yes_or_no, {"default": False})
        ...
        if(menu_entry_index == 8):
            print("Generating with given settings...\n")
            return settings
        if(menu_entry_index == 9):
            sys.exit()
